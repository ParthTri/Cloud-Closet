...
const API_URL = 'http://192.168.1.21:3000/api/image';
//const USER_ID = '3FB87276-7269-EF11-BDFD-000D3AD087AD';
const CATEGORIES_API_URL = 'http://192.168.1.21:3000/api/categories';
...
export default function Upload() {
    ...
    type ImageMimeType = 'image/jpeg' | 'image/png' | 'image/gif' | 'image/webp' | 'image/svg+xml' | 'image/bmp';
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        alert(`Upload failed: ${error.response?.data?.message || 'Unknown Axios error'}`);
  ...
  React.useEffect(() =>
{
  ...
  return (
    <ScrollView contentContainerStyle = {{padding: 20}}>
      ...
      <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 16 }}>Please select at least 1 category</Text>
      <FlatList
        data={categories}
